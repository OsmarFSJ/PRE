...
X = [float(dat[i][0]) for i in range(len(dat))]
Y = [float(dat[i][1])*pow(2, 5) for i in range(len(dat))]


plt.errorbar(X, Y, yerr=None, xerr=None, capthick=2, capsize=5, label='$L$ = 5', color='tab:blue', marker='o', linewidth=1)


arq = open('C_L-10.dat', 'r')
dat = arq.readlines()
dat.pop(0)
dat = [dat[i].split() for i in range(len(dat))]
arq.close()
...
X = [float(dat[i][0]) for i in range(len(dat))]
Y = [float(dat[i][1])*pow(2, 10) for i in range(len(dat))]


plt.errorbar(X, Y, yerr=None, xerr=None, capthick=2, capsize=5, label='$L$ = 10', color='tab:green', marker='o', linewidth=1)


arq = open('C_L-12.dat', 'r')
dat = arq.readlines()
dat.pop(0)
dat = [dat[i].split() for i in range(len(dat))]
arq.close()
...
X = [float(dat[i][0]) for i in range(len(dat))]
Y = [float(dat[i][1])*pow(2, 12) for i in range(len(dat))]


plt.errorbar(X, Y, yerr=None, xerr=None, capthick=2, capsize=5, label='$L$ = 12', color='orangered', marker='o', linewidth=1)


plt.xlabel('$n$', size=BIG)
plt.ylabel('number of local optima', size=BIG, labelpad=BIG)
plt.xticks(size=SMA)
plt.yticks(size=SMA)
plt.xlim([2, 15])
#plt.ylim()
