...
int main()
{
	...
	for(int i=0; i<sam.lan_N; i++)					// landscape generation loop
	{
		...
		for(int j=0; j<sam.dyn_N; j++)					// complete dynamics generation loop
		{
			wal.pos = 0;
			
			
			fprintf(data, "%d", wal.pos);
			
			
			for(int k=1; k<rou.len; k++)					// outer dynamics: global optimum displacement step
			{
				...
				for(int l=0; l<P(gen.N); l++)
					wal.pat[l] = P(gen.N);


				for(int l=0; !top.loc_max[wal.pos] && l<P(gen.N); l++)		// inner dynamics: stationary global optimum
				{
				...
				for(int l=0; wal.pat[l]!=P(gen.N) && l<P(gen.N) ; l++)
					fprintf(data, "\t%d", wal.pat[l]);
			}
